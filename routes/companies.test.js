...
describe("POST /companies", function () {
  ...
  test("unathorized if not admin users", async function () {
    const resp = await request(app)
      .post("/companies")
      .send(newCompany)
...
}); //TODO: add anon uses, if users make requests get 401 not 400
...
describe("GET /companies", function () {
  ...
  test("works for 1 param, returns filtered results", async function () {
  ...
  test("works if filter returns no results", async function () {
    const resp = await request(app).get("/companies?minEmployees=4");
...
//TODO: add deeper security checks to make sure sure unathorized users get back
//      401 requests and not 404 requests